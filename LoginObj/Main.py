...
def Cadastrar():
    ...
    DataNas = Cx_Cad_DataNas.get()
    Senha = Cx_Cad_Senha.get()
    Senha2 = Cx_Cad_ConfSenha.get()
    if Login and Nome and CPF and Genero!="Selecione" and DataNas and Senha:
        if Senha == Senha2:
             CadastroBD(Login,Nome, CPF, Genero, DataNas, Senha)
             LimparCax()
             Lb_Cad_Erro.configure(text="Cadastro Efetuado com Sucesso!")
        else:
            Lb_Cad_Erro.configure(text="Confira a senha!")
    else:
        Lb_Cad_Erro.configure(text="Confira suas informações!")
...
def Autenticar():
    Login1 = Cx_Login.get()
    Senha1 = Cx_Senha.get()
    Resposta = AutenticarBD(Login1, Senha1)
    match Resposta:
        case "Certo":
